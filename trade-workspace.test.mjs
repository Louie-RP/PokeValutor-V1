...
assert.match(searchMarkup, /styles\.css\?v=20260921-search-loading-3/);
...
assert.match(await readFile('styles.css', 'utf8'), /pv-tradeCarouselPage[\s\S]*grid-auto-rows: max-content[\s\S]*align-content: start/);
assert.match(await readFile('styles.css', 'utf8'), /pv-trade-grid > \.pv-tradeCarouselPage[\s\S]*display: contents/);
assert.match(searchSource, /tradeApi\.getConditionMarketValues\(prices\)/);
assert.match(cardSource, /tradeApi\.getConditionMarketValues\(prices\)/);
